    ...
    def execute(self):
        ...
        for goal in automations:
            ...
            self.repo.create_virtual_transaction(
                goal_id=goal.id, amount=goal.automation_amount, label=f"Auto-save: {goal.id}"
            )
            # 3. Update the next run date based on frequency
            new_date = self._calculate_next_date(goal.automation_next_run_date, goal.automation_frequency)
