...
def accounts_list(
    ...
    """Get all accounts for the user (both Powens and manual)."""
    # Fetch data
    ...
    for acc in manual_accs:
        ...
        acc_data["last_sync"] = acc.updated_at
        unified_list.append(acc_data)
...
def create_loan_account(
    ...
    return ManualAccountRead.model_validate(account)
...
def create_savings_account(
    ...
    return ManualAccountRead.model_validate(created_account)
...
async def import_savings_from_csv(
    ...
    snapshot_repo: SnapshotAccountRepository = Depends(
...
def get_account(
    ...
    return ManualAccountRead.model_validate(accounts[0])
...
def update_account(
    ...
    return ManualAccountRead.model_validate(updated)
...
def get_loan_balance(
    ...
    if account.account_type != "loan":
        raise HTTPException(
            status_code=400, detail="Balance calculation only for loans"
        )
...
def check_snapshot_reminder() -> dict:
    ...
    return {
        ...
        "message": "Time for your monthly Net Worth Sunday!"
        if is_reminder_day
        else "Not a snapshot day",
    }
